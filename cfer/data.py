...
def get_dataset_dev(config=BaseConfig, transform=None):
    """
    get dev dataset
    ...
    """
    return AffWildDataset(config.DIR_IMG, config.DIR_DEV, transform=transform)


def get_dataloader_train(config=BaseConfig, batch_size=32, shuffle=True, transform=None, image_size=224):
...
def get_dataloader_dev(config=BaseConfig, batch_size=32, shuffle=True, transform=None, image_size=224):
    if transform is None:
        transform = transforms.Compose([
...
if __name__ == '__main__':
    DIR_IMG = BaseConfig.DIR_IMG
    DIR_ANN_TRAIN = BaseConfig.DIR_ANN_TRAIN
    DIR_ANN_DEV = BaseConfig.DIR_ANN_DEV
    df = dir_to_df(DIR_IMG, DIR_ANN_TRAIN)
    print(df.head())
    make_label_file(DIR_IMG, DIR_ANN_TRAIN, 'labels.csv')
